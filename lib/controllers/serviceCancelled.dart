...
class ServiceRepositoryCancelled {
  ...
  Future<List<ServiceRequest>> fetchServicesByCancelled(String status, String userId, String column, String token) async {
    return await _fetch(status, userId);
  }

  Future<List<ServiceRequest>> _fetch(String status, String userId) async {
    try {
      print('[Cancelled] workerId=$userId status=$status');
      final snap = await firestore.collection('services')
          .where('workerId', isEqualTo: userId)
          .where('status', isEqualTo: status)
          .get();
      print('[Cancelled] docs=${snap.docs.length}');
      ...
      await Future.wait(list.map((svc) async {
        try {
          ...
          if (os.docs.isNotEmpty) {
            ...
            final oSt = o['status']?.toString() ?? 'cancelled';
            svc.offers = [Offer(
              id: o['id']?.toString() ?? '',
              workerId: userId,
        ...
        } catch (e) { print('[Cancelled] oferta err: $e'); }
      }));

      list.forEach(LocalCacheService.cacheServiceRequest);
      return list;
    } catch (e) {
      print('[Cancelled] Firestore err: $e');
      return [];
    }
  }
